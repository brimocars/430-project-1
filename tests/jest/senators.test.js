...
describe('getResponseStringPartFromParams', () => {
  ...
  it('tests with all parameters', () => {
    ...
    expect(dataHandler.getResponseStringPartFromParams(firstName, lastName, party, state))
      .toBe(expectedReturn);
  });
  it('tests with first name', () => {
    const expectedReturn = ' with first name \'Andy\'';
    expect(dataHandler.getResponseStringPartFromParams(firstName, undefined, undefined, undefined))
      .toBe(expectedReturn);
  });
  it('tests with last name', () => {
    const expectedReturn = ' with last name \'Kim\'';
    expect(dataHandler.getResponseStringPartFromParams(undefined, lastName, undefined, undefined))
      .toBe(expectedReturn);
  });
  it('tests with party name', () => {
    const expectedReturn = ' with party \'democrat\'';
    expect(dataHandler.getResponseStringPartFromParams(undefined, undefined, party, undefined))
      .toBe(expectedReturn);
  });
  it('tests with state name', () => {
    const expectedReturn = ' with state \'NJ\'';
    expect(dataHandler.getResponseStringPartFromParams(undefined, undefined, undefined, state))
      .toBe(expectedReturn);
  });
  it('tests with first name and state', () => {
    const expectedReturn = ' with first name \'Andy\' and state \'NJ\'';
    expect(dataHandler.getResponseStringPartFromParams(firstName, undefined, undefined, state))
      .toBe(expectedReturn);
  });
  it('tests with no parameters', () => {
    const expectedReturn = '';
    expect(dataHandler.getResponseStringPartFromParams(undefined, undefined, undefined, undefined))
      .toBe(expectedReturn);
  });
});
...
describe('getSenator', () => {
  // Note: these tests are kinda fragile. If the senators data ever needs to change, these will
  // need to be updated to. Ideally, if the senator data was stored in a database or something, a
  // mock db would be used for these tests, and custom testing senators could be added and checked
  // for. But, I'm not going that above and beyond, and this is sufficient for the purposes of this
  // project.
  ...
  it('tests getSenator', () => {
    ...
    expect(dataHandler.getSenator(firstName, lastName, undefined, undefined))
      .toHaveLength(expectedReturnLength);
  });
  it('tests getSenator', () => {
    const expectedReturnLength = 10;
    expect(dataHandler.getSenator(firstName, undefined, undefined, undefined))
      .toHaveLength(expectedReturnLength);
  });
  it('tests getSenator', () => {
    const expectedReturnLength = 45;
    expect(dataHandler.getSenator(undefined, undefined, party, undefined))
      .toHaveLength(expectedReturnLength);
  });
  it('tests getSenator', () => {
    const expectedReturnLength = 2;
    expect(dataHandler.getSenator(undefined, undefined, undefined, state))
      .toHaveLength(expectedReturnLength);
  });
  it('tests getSenator', () => {
    const expectedReturnLength = 100;
    expect(dataHandler.getSenator(undefined, undefined, undefined, undefined))
      .toHaveLength(expectedReturnLength);
  });
  it('tests getSenator', () => {
    const expectedReturnLength = 0;
    expect(dataHandler.getSenator(firstName, undefined, undefined, state))
      .toHaveLength(expectedReturnLength);
  });
});
...
describe('maybeSetName', () => {
  ...
  it('returns a senator\'s current name if it exists', () => {
    const { name } = senator.person;
    expect(dataHandler.maybeSetName(senator)).toBe(name);
  });
  it('returns a senator\'s name when one is not set', () => {
    const { name } = senator.person;
    delete senator.person.name;
    expect(senator.person.name).toBeUndefined();
    expect(dataHandler.maybeSetName(senator)).toBe(name);
  ...
  it('returns undefined and doesn\'t throw an error when necessary properties are missing', () => {
    // any validation error is handled in validateSenator, so maybeSetName never throws an error.
    // It just returns undefined.
    delete senator.person;
    expect(dataHandler.maybeSetName(senator)).toBeUndefined();
  });
});
